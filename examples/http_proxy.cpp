...
//     char *plaintext_buffer = (char *)pt2.c_str();
    ...
    if (data == 0 || dataLength == 0)
    {
        ...
        data = make_request(hostname_ip, buffer, requestTarget, &dataLength);
    ...
    cout<<"write data:"<<endl<<(char *)data<<endl;
...
/*    
  ...
  */  
   ...
   // if (write(sock, cipher_request, strlen(cipher_request)) != strlen(cipher_request))
    ...
    pfd.events = POLLIN | POLLRDNORM | POLLRDBAND; //  POLLIN 普通或优先级带数据可读，POLLRDNORM 普通数据可读，POLLRDBAND优先级带数据可读
    pfd.fd = sock;
