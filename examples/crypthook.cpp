...
static int encrypt_data(char *in, int len, char *out)
{
 ...
 //    printf("%x\n",out[0]);
    ...
    // cout << "encrypt step:" << endl << step << endl;//
...
//LD_PRELOAD=./crypthook.so python3 -m http.server 9000
//LD_PRELOAD=./crypthook.so curl http://127.0.0.1:9000

// cc crypthook.c -o crypthook.so -Wall -shared -fpic -ldl -lcrypto -L/usr/local/lib

// g++ -std=c++11 crypthook.cpp -o crypthook.so -Wall -shared -fpic -ldl -lcrypto -lrelic -lrelic_ec -lopenabe -L/usr/local/lib -I${ZROOT}/deps/root/include -I${ZROOT}/root/include -L${ZROOT}/deps/root/lib -L${ZROOT}/root/lib
